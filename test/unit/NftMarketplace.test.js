...
!developmentChains.includes(network.name)
    ...
    : describe("Nft Marketplace Unit Tests", function () {
          ...
          beforeEach(async () => {
              ...
              // Using getNamedAccounts
              deployer = (await getNamedAccounts()).deployer; // deployer account
              user = (await getNamedAccounts()).user; // user account

              // Deploy all contracts
              await deployments.fixture(["all"]);

              // Get deployed contracts
              // 1) Deployer deploy contracts
              // nftMarketplaceContract = await ethers.getContract("NftMarketplace");
              // nftMarketplace = nftMarketplaceContract.connect(deployer);
              // nftMarketplaceContract = await ethers.getContract("NftMarketplace" /*deployer*/); // 'deployer' here can be omitted - ethers takes 1st account by default
              // 2) User deploy contracts
              // nftMarketplaceContract = await ethers.getContract("NftMarketplace");
              // nftMarketplace = nftMarketplaceContract.connect(user);
              // nftMarketplaceContract = await ethers.getContract("NftMarketplace", user);
              //
          ...
          describe("listItem", function () {
              // This only test was written in the course
              ...
              it("needs approvals to list item", async function () {
                  ...
                  await expect(
                      nftMarketplace.listItem(basicNft.address, TOKEN_ID, PRICE)
                  ).to.be.revertedWith("NftMarketplace__NotAprovedForMarketplace");
              });
